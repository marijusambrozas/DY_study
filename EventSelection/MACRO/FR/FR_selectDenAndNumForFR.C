...
// -- Muon Channel -- //
// Started: 22 Jul 2018
// Modify the binning of histograms: 10 Oct 2018
void FR_selectDenAndNumForFR(Int_t debug, Int_t type, Int_t remainder = 9999, Int_t isTopPtReweighting = 0, TString HLTname = "Mu50")
{
	...
	// -- Output ROOTFile -- //	
        ...
        TString Output_ROOTFile = "../../RESULT/FR/hist_TightID_PFIso_20200129_"+TString::Itoa(type,10)+"_"+TString::Itoa(remainder,10)+"_"
	...
	for(Int_t i_tup = 0; i_tup<Ntup; i_tup++)
	{
		...
		// -- Making Histogram -- //
		////////////////////////////
		TH1D* denominator_pt = new TH1D("denominator_pt_"+Tag[i_tup],"",948,52,1000); 
		TH1D* denominator_pt_barrel = (TH1D*)denominator_pt->Clone("denominator_pt_barrel_"+Tag[i_tup]);
		TH1D* denominator_pt_endcap = (TH1D*)denominator_pt->Clone("denominator_pt_endcap_"+Tag[i_tup]);  

		TH1D* numerator_pt = (TH1D*)denominator_pt->Clone("numerator_pt_"+Tag[i_tup]);
		TH1D* numerator_pt_barrel = (TH1D*)denominator_pt->Clone("numerator_pt_barrel_"+Tag[i_tup]);
		TH1D* numerator_pt_endcap = (TH1D*)denominator_pt->Clone("numerator_pt_endcap_"+Tag[i_tup]);

		TH1D* denominator_eta = new TH1D("denominator_eta_"+Tag[i_tup],"",48,-2.4,2.4);
		TH1D* numerator_eta = (TH1D*)denominator_eta->Clone("numerator_eta_"+Tag[i_tup]);

		// vs. isolation
		TH1D* denominator = new TH1D("denominator_"+Tag[i_tup],"",1000,0,5);
		TH1D* denominator_barrel = (TH1D*)denominator->Clone("denominator_barrel_"+Tag[i_tup]);
		TH1D* denominator_endcap = (TH1D*)denominator->Clone("denominator_endcap_"+Tag[i_tup]);

		TH1D* numerator = (TH1D*)denominator->Clone("numerator_"+Tag[i_tup]);
		TH1D* numerator_barrel = (TH1D*)denominator->Clone("numerator_barrel_"+Tag[i_tup]);
		TH1D* numerator_endcap = (TH1D*)denominator->Clone("numerator_endcap_"+Tag[i_tup]);
			...
			if( TriggerFlag == kTRUE && GenFlag == kTRUE && GenFlag_top == kTRUE )
			{
				...
				for(Int_t j=0; j<(int)MuonCollection.size(); j++)
				{
                                                ...
                                                && mu_.Pt > 52 && fabs(mu_.eta) < 2.4 )  // HE FORGOT FABS(ETA)!!!!!
					{
						...
						Double_t iso = mu_.RelPFIso_dBeta;
						...
						//if( iso > 0.1 ) continue; //for trkiso
						if( iso > 0.15 ) continue; //for RelPFIso_dBeta
