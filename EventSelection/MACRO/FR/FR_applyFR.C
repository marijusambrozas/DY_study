...
void FR_applyFR(Int_t debug, Int_t type, Int_t remainder = 9999, Int_t isTopPtReweighting = 0, TString HLTname = "IsoMu24_OR_IsoTkMu24")
{
	...
	for(Int_t i_tup = 0; i_tup<Ntup; i_tup++)
	{
		...
		TH1D* histDijet2 = (TH1D*)histDijet1->Clone("histDijet2_"+Tag[i_tup]);
		TH1D* histSameDijet1 = (TH1D*)histDijet1->Clone("histSameDijet1_"+Tag[i_tup]);
		TH1D* histSameDijet2 = (TH1D*)histDijet1->Clone("histSameDijet2_"+Tag[i_tup]);

		TH1D* fitDijet1 = new TH1D("fitDijet1_"+Tag[i_tup],"",37,15,200);
		TH1D* fitDijet2 = (TH1D*)fitDijet1->Clone("fitDijet2_"+Tag[i_tup]);
		TH1D* fitSameDijet1 = (TH1D*)fitDijet1->Clone("fitSameDijet1_"+Tag[i_tup]);
		TH1D* fitSameDijet2 = (TH1D*)fitDijet1->Clone("fitSameDijet2_"+Tag[i_tup]);

		TH1D* rapDijet1 = new TH1D("rapDijet1_"+Tag[i_tup],"",48,-2.4,2.4);
		TH1D* rapDijet2 = (TH1D*)rapDijet1->Clone("rapDijet2_"+Tag[i_tup]);
		TH1D* rapSameDijet1 = (TH1D*)rapDijet1->Clone("rapSameDijet1_"+Tag[i_tup]);
		TH1D* rapSameDijet2 = (TH1D*)rapDijet1->Clone("rapSameDijet2_"+Tag[i_tup]);

		// -- Wjet -- //
		TH1D* histWJets1 = (TH1D*)histDijet1->Clone("histWJets1_"+Tag[i_tup]);
		TH1D* histWJets2 = (TH1D*)histDijet1->Clone("histWJets2_"+Tag[i_tup]);
		TH1D* histSameWJets1 = (TH1D*)histDijet1->Clone("histSameWJets1_"+Tag[i_tup]);
		TH1D* histSameWJets2 = (TH1D*)histDijet1->Clone("histSameWJets2_"+Tag[i_tup]);

		TH1D* fitWJets1 = (TH1D*)fitDijet1->Clone("fitWJets1_"+Tag[i_tup]);
		TH1D* fitWJets2 = (TH1D*)fitDijet1->Clone("fitWJets2_"+Tag[i_tup]);
		TH1D* fitSameWJets1 = (TH1D*)fitDijet1->Clone("fitSameWJets1_"+Tag[i_tup]);
		TH1D* fitSameWJets2 = (TH1D*)fitDijet1->Clone("fitSameWJets2_"+Tag[i_tup]);

		TH1D* rapWJets1 = (TH1D*)rapDijet1->Clone("rapWJets1_"+Tag[i_tup]);
		TH1D* rapWJets2 = (TH1D*)rapDijet1->Clone("rapWJets2_"+Tag[i_tup]);
		TH1D* rapSameWJets1 = (TH1D*)rapDijet1->Clone("rapSameWJets1_"+Tag[i_tup]);
		TH1D* rapSameWJets2 = (TH1D*)rapDijet1->Clone("rapSameWJets2_"+Tag[i_tup]);

		// -- for check -- //
		TH1D* histPass = new TH1D("histPass_"+Tag[i_tup],"",10,0,10);
		TH1D* histFail = new TH1D("histFail_"+Tag[i_tup],"",10,0,10);

		TH1D* histDijet = (TH1D*)histDijet1->Clone("histDijet_"+Tag[i_tup]);
		TH1D* histSameDijet = (TH1D*)histDijet1->Clone("histSameDijet_"+Tag[i_tup]);
		TH1D* fitDijet = (TH1D*)fitDijet1->Clone("fitDijet_"+Tag[i_tup]);
		TH1D* fitSameDijet = (TH1D*)fitDijet1->Clone("fitSameDijet_"+Tag[i_tup]);
		TH1D* rapDijet = (TH1D*)rapDijet1->Clone("rapDijet_"+Tag[i_tup]);
		TH1D* rapSameDijet = (TH1D*)rapDijet1->Clone("rapSameDijet_"+Tag[i_tup]);

		TH1D* histWJets = (TH1D*)histDijet1->Clone("histWJets_"+Tag[i_tup]);
		TH1D* histSameWJets = (TH1D*)histDijet1->Clone("histSameWJets_"+Tag[i_tup]);
		TH1D* fitWJets = (TH1D*)fitDijet1->Clone("fitWJets_"+Tag[i_tup]);
		TH1D* fitSameWJets = (TH1D*)fitDijet1->Clone("fitSameWJets_"+Tag[i_tup]);
		TH1D* rapWJets = (TH1D*)rapDijet1->Clone("rapWJets_"+Tag[i_tup]);
		TH1D* rapSameWJets = (TH1D*)rapDijet1->Clone("rapSameWJets_"+Tag[i_tup]);
		...
		{	
			...
			if( TriggerFlag == kTRUE && GenFlag == kTRUE && GenFlag_top == kTRUE )
			{
				...
				for(Int_t j=0; j<(int)MuonCollection.size(); j++)
				{
					...
					if( mu_.isTightMuon() && mu_.Pt > 17 && mu_.eta < 2.4 )
					{
						if( mu_.Pt > 28 ) flag_lead = kTRUE;

						//if( mu_.trkiso < 0.1 )
						if( mu_.RelPFIso_dBeta < 0.15 )
							passMuons.push_back(MuonCollection[j]);
						else
							failMuons.push_back(MuonCollection[j]);
					}
				}

				if( flag_lead == kFALSE ) continue;

				histPass->Fill(passMuons.size(), TotWeight);
				histFail->Fill(failMuons.size(), TotWeight);

				nPass += TotWeight * passMuons.size();
				nFail += TotWeight * failMuons.size();
				...
				if( isPassEventSelection_Dijet == kTRUE )
				{
					...
					Double_t weight_template = TotWeight * PUWeight * effweight * FR1_template * FR2_template / ((1-FR1_template) * (1-FR2_template));
					Double_t weight_ratio = TotWeight * PUWeight * effweight * FR1_ratio * FR2_ratio / ((1-FR1_ratio) * (1-FR2_ratio));

					if( mu1.charge != mu2.charge )
					{
						if( mass > 15 && mass < 3000)
						{
							//histDijet->Fill(mass, TotWeight * PUWeight);
							histDijet->Fill(mass, TotWeight * PUWeight * effweight);
							histDijet1->Fill(mass, weight_template);
							histDijet2->Fill(mass, weight_ratio);

							//fitDijet->Fill(mass, TotWeight * PUWeight);
							fitDijet->Fill(mass, TotWeight * PUWeight * effweight);
							fitDijet1->Fill(mass, weight_template);
							fitDijet2->Fill(mass, weight_ratio);

							//rapDijet->Fill(rap, TotWeight * PUWeight);
							rapDijet->Fill(rap, TotWeight * PUWeight * effweight);
					...
					else
					{
						if( mass > 15 && mass < 3000)
						{
							//histSameDijet->Fill(mass, TotWeight * PUWeight);
							histSameDijet->Fill(mass, TotWeight * PUWeight * effweight);
							histSameDijet1->Fill(mass, weight_template);
							histSameDijet2->Fill(mass, weight_ratio);

							//fitSameDijet->Fill(mass, TotWeight * PUWeight);
							fitSameDijet->Fill(mass, TotWeight * PUWeight * effweight);
							fitSameDijet1->Fill(mass, weight_template);
							fitSameDijet2->Fill(mass, weight_ratio);

							//rapSameDijet->Fill(rap, TotWeight * PUWeight);
							rapSameDijet->Fill(rap, TotWeight * PUWeight * effweight);
				...
				if( isPassEventSelection_Wjet == kTRUE )
				{
					...
					Double_t weight_template = TotWeight * PUWeight * effweight * FR2_template / (1-FR2_template);
					Double_t weight_ratio = TotWeight * PUWeight * effweight * FR2_ratio / (1-FR2_ratio);

					if( mu1.charge != mu2.charge )
					{
							if( mass > 15 && mass < 3000)
							{
								//histWJets->Fill(mass, TotWeight * PUWeight);
								histWJets->Fill(mass, TotWeight * PUWeight * effweight);
								histWJets1->Fill(mass, weight_template);
								histWJets2->Fill(mass, weight_ratio);

								//fitWJets->Fill(mass, TotWeight * PUWeight);
								fitWJets->Fill(mass, TotWeight * PUWeight * effweight);
								fitWJets1->Fill(mass, weight_template);
								fitWJets2->Fill(mass, weight_ratio);

								//rapWJets->Fill(rap, TotWeight * PUWeight);
								rapWJets->Fill(rap, TotWeight * PUWeight * effweight);
					...
					else
					{
							if( mass > 15 && mass < 3000)
							{
								//histSameWJets->Fill(mass, TotWeight * PUWeight);
								histSameWJets->Fill(mass, TotWeight * PUWeight * effweight);
								histSameWJets1->Fill(mass, weight_template);
								histSameWJets2->Fill(mass, weight_ratio);

								//fitSameWJets->Fill(mass, TotWeight * PUWeight);
								fitSameWJets->Fill(mass, TotWeight * PUWeight * effweight);
								fitSameWJets1->Fill(mass, weight_template);
								fitSameWJets2->Fill(mass, weight_ratio);

								//rapSameWJets->Fill(rap, TotWeight * PUWeight);
								rapSameWJets->Fill(rap, TotWeight * PUWeight * effweight);
